...
public class DungeonGenerator : MonoSingleton <DungeonGenerator> {
	...
	public override void Init() {
		...
		for (int i = 0; i < MAP_HEIGHT; i++) {
			for (int j = 0; j < MAP_WIDTH; j++) {
				tiles[i,j] = new DungeonTile(TileType.EMPTY);
			}
		}
	...
	public void GenerateDungeon(int seed) {
		...
		if (debugToTexture) { floor.GetComponent<Renderer>().material.mainTexture = quadTree.QuadTreeToTexture(); }
		...
		if (debugToTexture) { floor.GetComponent<Renderer>().material.mainTexture = dungeonTexture; }
		...
		if (debugToTexture) {
			dungeonTexture = DungeonToTexture();
			floor.GetComponent<Renderer>().material.mainTexture = dungeonTexture;
